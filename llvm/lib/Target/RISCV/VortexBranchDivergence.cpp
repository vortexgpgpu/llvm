...
// Complexity guard — the structurization + divergence pipeline (StructurizeCFG
// in particular) has worst-case exponential behavior on large/complex CFGs.
// musl printf-family (vfprintf=422 BBs, iconv=227, vfwprintf=149) hangs for
// minutes-hours. Skip the divergence pipeline for functions over this BB
// count — critical SIMT math (sinf=25, sin/cos=13) is well under. Such large
// functions are usually I/O / parsing / crypto that aren't called from SIMT.
static cl::opt<unsigned> VortexMaxDivergenceBBs(
    "vortex-divergence-max-bbs",
    cl::desc("Skip the Vortex divergence pipeline for functions with more "
...
bool VortexDivergenceAnalysis0::runOnModule(Module &M) {
  ...
  auto &TM = TPC.getTM<CodeGenTargetMachineImpl>();
  ...
  // Traverse call graph in reverse post-order; execute analysis passes on each
  // function until convergence. Skip pathologically-complex functions (musl's
  // vfprintf-class, hundreds of BBs in one region) — their structurization is
  // exponential anyway and downstream VortexBranchDivergence0/1 will also bail
  // (via the same VortexMaxDivergenceBBs guard). Also cap the fixed-point
  // iteration count to avoid pathological convergence on large modules.
  bool changed;
  unsigned iter = 0;
  static constexpr unsigned kMaxFixedPointIterations = 16;
  do {
    changed = false;
    for (auto it = funcs.rbegin(), ie = funcs.rend(); it != ie; ++it) {
      if ((*it)->size() > VortexMaxDivergenceBBs) {
        LLVM_DEBUG(dbgs() << "*** VX: skip large function in analysis: "
                          << (*it)->getName() << " (" << (*it)->size()
                          << " BBs)\n");
        continue;
      }
    ...
    if (++iter >= kMaxFixedPointIterations) {
      LLVM_DEBUG(dbgs() << "*** VX: hit fixed-point iteration cap ("
                        << kMaxFixedPointIterations << ")\n");
      break;
    }
  } while (changed);
...
bool VortexBranchDivergence0::runOnFunction(Function &F) {
  ...
  // Complexity guard — see VortexMaxDivergenceBBs at top of file. Real perf
  // fix (NOT cosmetic): the structurization + divergence pipeline is
  // worst-case exponential on large CFGs.
  if (F.size() > VortexMaxDivergenceBBs) {
...
bool VortexBranchDivergence1::runOnFunction(Function &F) {
  ...
  // Complexity guard — same threshold as VBD0. Real perf fix (NOT cosmetic):
  // StructurizeCFG was skipped on large functions; emitting split/join here
  // without prior structurization produces dominance violations.
  if (F.size() > VortexMaxDivergenceBBs) {
    LLVM_DEBUG(dbgs() << "*** VX: skip divergence on " << F.getName()
                      << " (" << F.size() << " BBs > "
                      << VortexMaxDivergenceBBs << ")\n");
