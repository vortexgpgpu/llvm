...
K_CFLAGS="-v -O3 --sysroot=${SYSROOT} --gcc-toolchain=${RISCV_TOOLCHAIN_PATH} -march=rv32imf -mabi=ilp32f -Xclang -target-feature -Xclang +vortex -I${VORTEX_PATH}/hw -I${VORTEX_RT_PATH}/include -fno-rtti -fno-exceptions -ffreestanding -nostartfiles -fdata-sections -ffunction-sections"
...
{
    rm -f ${TEST}.ast
    ${LLVM_PREFIX}/bin/clang -O0 -g -v -Xclang -ast-dump -fsyntax-only ${TEST}.c > ${TEST}.ast
}
...
llvm_ir()
{
    ...
    ${LLVM_PREFIX}/bin/clang ${K_CFLAGS} -S -emit-llvm ${TEST}.c > ${TEST}.ll
    
    ${LLVM_PREFIX}/bin/opt -dot-cfg -S ${TEST}.ll
...
{
    rm -f ${TEST}.s
    #${LLVM_PREFIX}/bin/llc ${K_LLCFLAGS} --print-after-all -filetype=asm ${TEST}.ll -o ${TEST}.s
...
codegen()
{
    ...
    ${LLVM_PREFIX}/bin/clang ${K_CFLAGS} ${TEST}.bc -c -o ${TEST}.o -mllvm -debug-pass=Arguments > build.log 2>&1
